...
public class WhatIfComparison {
    ...
    public WhatIfComparison(TripPlan oldPlan,
        ...
        calculateDifferences(dailyHours);
    }

    private void calculateDifferences(int dailyHours) {
        ...
        int oldDays = calculateDays(oldPlan.getTotalTime(), dailyHours);
        int newDays = calculateDays(newPlan.getTotalTime(), dailyHours);

        dayDifference = newDays - oldDays;
    }

    private int calculateDays(int totalTime, int dailyHours) {
        if (dailyHours <= 0) {
            return 1;
        }

        int dailyLimitMinutes = dailyHours * 60;
        return (int) Math.ceil((double) totalTime / dailyLimitMinutes);
    }
